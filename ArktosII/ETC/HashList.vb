'' IN ORDER TO USE IT AS ANOTHER COLLECTION
'' A) Replace "HashList"   ==> "CollectionName"
'' B) Replace "HashMember" ==> "CollectionItem"
...
#Region " Custom Public Methods "
    ...
    Public Sub Remove(ByVal value As HashMember)
        myHash.Remove(value.ID)
        myList.Remove(value)
    End Sub

    Default Public Property Item(ByVal ID As Guid) As HashMember
        Get
            Item = CType(myHash.Item(ID), HashMember)
        End Get
        Set(ByVal Value As HashMember)
    ...
    Public Function ContainsValue(ByVal value As HashMember) As Boolean
        Return myHash.ContainsValue(value)
    End Function

    Public Sub Add(ByVal value As HashMember)
        myHash.Add(value.ID, value)
        myList.Add(value)
    End Sub

    Public Function Clone() As HashList
        Dim Cloned As New HashList
        Dim tListItem As HashMember
...
#Region " From IList "
    ...
    Public Function IndexOf(ByVal value As HashMember) As Integer
        Return myList.IndexOf(value)
    End Function

    Public Sub Insert(ByVal index As Integer, ByVal value As HashMember)
        myList.Insert(index, value)
        myHash.Add(value.ID, value)
    End Sub

    Default Public Property Item(ByVal index As Integer) As HashMember
        Get
            Item = CType(myHash(myList(index)), HashMember)
        End Get
        Set(ByVal Value As HashMember)
    ...
    Public Sub RemoveAt(ByVal index As Integer)
        myHash.Remove(CType(myList(index), HashMember).ID)
...
'Public Function KeyOf(ByVal value As HashMember) As Guid
'    Dim tDictionaryEntry As DictionaryEntry
'    For Each tDictionaryEntry In myHash
'        If (tDictionaryEntry.Value Is value) Then
'            Return tDictionaryEntry.Key
'        End If
'    Next
'End Function